...
export(int) var lines_per_frame := 10
...
func _process(_delta: float) -> void:
	...
	for _index in lines_per_frame:
		line_container.add_child(lines.pop_front())
		if lines.empty():
			break


func generate(start_position: Vector2, iterations: int, length_reduction: float, color: Color, width: float, rule: Rule) -> Array:
	var length = -200
...
class Rule:
	...
	func get_character(character: String) -> String:
		return rules.get(character, character)

	func get_action(character: String) -> String:
